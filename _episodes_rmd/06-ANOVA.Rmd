...
![Decision tree for comparisons between more than two groups](../fig/06-fig1.png)
...
The Shapiro-Wilk test for group 1 gives a significant p-value, indicating that we 
...
![Box plots of weight by alcohol consumption group](../fig/06-fig3.png)

We can see that with a p-value of 0.64, we reject the alternative hypothesis and
conclude that in this dataset, there is no evidence for a difference in 
...
**_Post-hoc_ test**, either Tukey's HSD for ANOVA or Dunn's test (in the 
...
groups 3 and 2. In this example, the p-values are all 1 - there is no evidence for 
even the slightest difference between the groups!

If there is a significant p-value with a one-way ANOVA, use Tukey's HSD test.
```{r}
TukeyHSD(result)
```
...
Prozac on the reported 'happiness score' of patients, and whether male and 
...
![Box plots of score by treatment and by gender](../fig/06-fig5.png)
...
![Box plots of score by treatment, filled by gender](../fig/06-fig6.png)
...
![Interaction diagram](../fig/06-fig7.png)
...
![Interaction plot of score by treatment and gender](../fig/06-fig8.png)
...
#### Gender
The p-value for gender is not significant, so there is no evidence for
...
![ANOVA model assumptions](../fig/06-fig9.png)
...
Smaller distances means that removing the observation has little effect on the 
