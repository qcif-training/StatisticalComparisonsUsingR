...
only accept if the data and analysis are sufficiently convincing.
...
1. We establish our null and alternative hypotheses.
...
H<sub>0</sub> when H<sub>0</sub> is true.
  + For instance, a significance level of 0.05 indicates there is a 5% risk 
  of incorrectly concluding a difference exists when there is no actual difference
  (often α=0.05, but can be as low as α=0.0000003 in some disciplines).
...
> > is true, rather that we don't have adequate justification to conclude 
> > otherwise.
> {: .solution}
{: .challenge}
...
II error). The probability of a type I error is given by the significance level; 
it is possible to calculate the probability of a type II error, but we will 
not cover that in this course.
![Error matrix](../fig/01-fig1.png)
