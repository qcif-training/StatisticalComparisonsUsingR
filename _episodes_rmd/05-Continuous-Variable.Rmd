...
![Decision tree for comparisons between two groups](../fig/05-fig1.png)
...
> > 
> > One dataset is normally distributed, the other is not, so we choose the 
...
less than the first). As we discussed in the introduction, generally a two-sided
...
Unless **both** groups are normally distributed, use the Mann-Whitney test 
(also known as the Wilcoxon rank-sum test). This is a non-parametric test analogous 
to the unpaired t-test, used when the _dependent_ variable is non-normally distributed.
...
> returning the Mann-Whitney test instead. Carry out a paired Wilcoxon test 
> (called the Wilcoxon signed-rank test) with the `paired = TRUE` argument.
{: .callout}
...
plot(gallstones$Height ~ gallstones$Gender, 
     ...
     xlab = 'Gender')
```
![Box plots of height by gender](../fig/05-fig2.png)
...
par(mfrow=c(1,1))
```
![Histograms of height by gender](../fig/05-fig3.png)

This doesn't look very normally-distributed, but we do have relatively few data
points. A more convincing way to determine this would be with the Shapiro-Wilk
test.
```{r}
...
## Paired samples
If data is paired, that is, they are the same samples under two different 
...
other), and the data must be in the same sample order in each group.
