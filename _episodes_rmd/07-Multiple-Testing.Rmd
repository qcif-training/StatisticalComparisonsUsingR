...
exercises: 45
questions:
- "How do we interpret p-values when multiple comparisons are carried out?"
objectives:
...
Each time we perform a t-test, we choose a significance threshold under which we
...
In our example three-way comparison with significance threshold of 0.05:
...
* Bonferroni correction (FWER): reduces the probability of even one false 
discovery
...
# False Discovery Rate correction - only three tests remain significant
...
This table can be used as a reference to select the appropriate test to use for
...
|Compare means or medians of two sample groups (paired data) |Paired t-test |Wilcoxon signed-rank (matched pairs) test |
...
|-----|-----|
|Compare two sample proportions of categorical data (unpaired data, ≤5 in one group) |Fisher's exact test |
...
* Wilcoxon signed-rank (matched pairs) test: `wilcox.test(var1~var2,paired=TRUE)`
...
> > either Chi-square or Fisher's exact test, depending on the expected count for 
> > each category.
> > ```r
